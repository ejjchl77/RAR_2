...
if True:  # Include project path
    ...
    import utils.lib_plot_crop as lib_plot
...
def get_dst_folder_name(src_data_type, src_data_path):
    ...
    if src_data_type == "video":  # /root/data/video.avi --> video
        ...
        _, folder_name = lib_commons.get_time() #EJ
...
CROP_SAVE_PATH = args.output_folder + "/crop" #EJ 0228
os.makedirs(CROP_SAVE_PATH, exist_ok=True) #EJ 0228
...
def draw_result_img(img_disp, ith_img, humans, dict_id2skeleton,
                    skeleton_detector, multiperson_classifier, crop_save_path, DST_FOLDER_NAME): # EJ: crop_save_path & DST_FOLDER_NAME & 
    ...
    desired_cols = int(1.0 * c * (img_disp_desired_rows / r)) # EJ: img_disp_desired_rows=480 from config file
    img_disp = cv2.resize(img_disp,
                          dsize=(desired_cols, img_disp_desired_rows)) # EJ: image size -> 480*480 비율로 변경함 

    img_origin = img_disp.copy()
    ...
    if len(dict_id2skeleton):
        for id, label in dict_id2label.items():
            ...
            #### EJ 수정 
            if label == 'sitting': new_label = '1.0 met'#'1.0 met (sitting)'
            elif label == 'stand': new_label = '1.2 met'#'1.2 met (stand)'
            elif label == 'walking': new_label = '1.7 met'#'1.7 met (walking)'
            elif label == 'exercise': new_label = '3.5 met (exercise)'
            elif label == 'sleeping': new_label = '0.7 met (sleeping)'
            else: new_label=label
            crop_save_path_= crop_save_path + '/id' + str(id)+'/'
            os.makedirs(crop_save_path_, exist_ok=True)
            crop_save_name = crop_save_path_ + lib_commons.get_time_string() + ".jpg"  # ejadd 0228
            lib_plot.draw_action_result(img_disp, id, skeleton, new_label, crop_save_name, img_origin)
    ...
    try:
        ...
        while images_loader.has_image():
            ...
            if (datetime.datetime.now()-Start_time).seconds <=20:
                ...
                # -- Track people
                dict_id2skeleton = multiperson_tracker.track(skeletons, img_disp)  # int id -> np.array() skeleton
                ...
                # -- Draw
                img_disp = draw_result_img(img_disp, ith_img, humans, dict_id2skeleton,skeleton_detector, multiperson_classifier, CROP_DST_FOLDER, DST_FOLDER_NAME)  # EJ: crop_save_path 추가 
                ...
                # -- Display image, and write to video.avi
                    ...
                    #crop image EJ
                    crop_img_sav_path = CROP_DST_FOLDER + lib_commons.get_time_string() + '.jpg' #EJ
                
                time.sleep(0.5)
            else:
                output_path = args.output_folder +"/output/"+ DST_FOLDER_NAME + "/"
                image_path = args.output_folder +"/image/"+ DST_FOLDER_NAME + "/"
                crop_path = args.output_folder +"/crop/" + DST_FOLDER_NAME + "/"
                output_check=os.listdir(output_path)
                image_check=os.listdir(image_path)
                crop_check=os.listdir(crop_path)
                
                if len(output_check) == 1: shutil.rmtree(output_path)
                if len(image_check) == 0: shutil.rmtree(image_path)
                if len(crop_check) == 0: shutil.rmtree(crop_path)
