...
def draw_action_result(img_display, id, skeleton, str_action_label, crop_save_name, img_origin):
    ...
    # EJ: bounding box image save
    gap = 20
    
    crop_startrow = 0 if (miny-gap) < 0 else miny-gap
    crop_startcol = 0 if (minx-gap) < 0 else minx-gap
    crop_endrow = img_display.shape[0] if (maxy+gap) > img_display.shape[0] else maxy+gap
    crop_endcol = img_display.shape[1] if (maxx+gap) > img_display.shape[1] else maxx+gap
    
    img_crop = img_origin[crop_startrow:crop_endrow, crop_startcol:crop_endcol] # EJ: [startrow:endrow, startcol:endcol]
    cv2.imwrite(crop_save_name, img_crop) # EJ: save cropped image
    
    # Draw text at left corner
    box_scale = max(
        0.5, min(2.0, (1.0*(maxx - minx)/img_display.shape[1] / (0.3))**(0.5)))
    fontsize = 0.8 * box_scale
    fontsize = 0.5
